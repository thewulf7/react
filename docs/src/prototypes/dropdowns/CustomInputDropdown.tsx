...
export interface CustomInputDropdownProps {}

class CustomInputDropdown extends React.Component<CustomInputDropdownProps, any> {
  private endHandler: any

  constructor(props: CustomInputDropdownProps) {
    super(props)
    this.state = {
      top: null,
      left: null,
      ...
      users: ['Charmander', 'Squirtle', 'Bulbasaur', 'Pikachu'],
  ...
  toggleSuggestor(metaInformation) {
    ...
    if (hookType === 'start') {
      this.setState({
        showSuggestor: true,
        left: cursor.left,
        top: cursor.top, // we need to add the cursor height so that the dropdown doesn't overlap with the `@`.
  ...
  handleKeyDown(event) {
    ...
    if (which === 13) {
      ...
      const newText = `${textareaValue.slice(0, startPosition - 1)}${user}${textareaValue.slice(
